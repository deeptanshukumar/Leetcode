...
public:
    vector<int> rotateElements(vector<int>& nums, int k) {
        ...
        for(auto i: nums){
            if(i>=0) nonnegative.push_back(i);
        }
        if (nonnegative.empty()) return nums;
        k = k%nonnegative.size();
        
        reverse(nonnegative.begin(), nonnegative.begin() + k);
        reverse(nonnegative.begin() + k, nonnegative.end());
